...
#     def test_if_main_ar_sets_correct_useragent_while_quering_iam(
#             self, main_ar_process_pertest, mocker, valid_user_header):
...
#         assert len(r_reqs) == 1
#         verify_header(r_reqs[0]['headers'], 'User-Agent', 'Main Admin Router')
...
class TestAuthnJWTValidator:
    ...
    def test_auth_token_not_provided(self, main_ar_process_perclass):
        log_messages = {
            "No auth token in request.": SearchCriteria(1, True),
            }

        assert_endpoint_response(
            main_ar_process_perclass, EXHIBITOR_PATH, 401, assert_error_log=log_messages)

    def test_invalid_auth_token_in_cookie(self, main_ar_process_perclass):
        log_messages = {
    ...
    def test_valid_auth_token(self, main_ar_process_perclass, valid_user_header):
        log_messages = {
...
class TestAuthCustomErrorPages:
    def test_correct_401_page_content(self, main_ar_process_pertest, repo_is_ee):
        url = main_ar_process_pertest.make_url_from_path(EXHIBITOR_PATH)
...
class TestAuthPrecedence:
    def test_if_service_endpoint_auth_precedence_is_enforced(
        ...
        url = main_ar_process_pertest.make_url_from_path("/service/i/do/not/exist")
        resp = requests.get(
            url,
    ...
    def test_if_agent_endpoint_auth_precedence_is_enforced(
        ...
        url = main_ar_process_pertest.make_url_from_path(uri)
        resp = requests.get(
            url,
            allow_redirects=False)
    ...
    def test_if_mleader_endpoint_auth_precedence_is_enforced(
        ...
        url = main_ar_process_pertest.make_url_from_path(
